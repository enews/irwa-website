 ...
 */
function createOverflowWrapper( { document, element, isPointerDevice, inheritedClasses } ) {
	if ( !element || !element.parentNode ) {
		console.error(
			'[Overflow] Element or element.parentNode is null or undefined.'
		);
		return null;
	}
	...
	try {
		...
		console.error(
			`[Overflow] Error occurred while wrapping element: ${ error.message }`
		);
		return null;
	}
}